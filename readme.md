...
A* ptr = new C();
```
Ставит ссылку на подкласс `A`, который лежит внутри класса `C`. Если деструктор `A` был бы не виртуальный, то вызвался бы только он. Однако, в нашем случае будут вызываться все деструкторы: сначала вызовется деструктор `C`, а затем деструктор `A` и `B`
```c++
~C()
~A()
~B()
...
Преобразованию типов в стиле Си эквивалентно преобразованиям `static_cast`. Тогда с наследованием все будет хорошо, то есть адреса будут грамотно отображаться.
