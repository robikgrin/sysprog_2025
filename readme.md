...
Решение всего задания представлено в папке `permanent`, где обертка функции из `c++` в `python` была выполнена с помощью `pybind11`. Сравнение результатов проведено в `some_check.ipynb`.